...
impl Atlas {
    ...
    /// Mark every layer empty so the atlas can be reused for a fresh upload
    /// without recreating the GPU texture. Caller must guarantee no in-flight
    /// frame still reads this atlas (use double-buffering for animated content).
