    ...
    P::Theme: theme::Base,
{
    ...
    struct Runner<Message: 'static, F> {
        ...
        /// Android: NativeWindow / surface is only valid after the first `resumed`.
        #[cfg(target_os = "android")]
        android_resumed: bool,
        #[cfg(target_os = "android")]
        pending_create: Option<Control>,
        #[cfg(target_os = "android")]
        force_control: Option<Control>,
    }
        ...
        fn resumed(&mut self, event_loop: &winit::event_loop::ActiveEventLoop) {
            ...
            #[cfg(target_os = "android")]
            {
                ...
                if let Some(control) = self.pending_create.take() {
                    ...
                    self.force_control = Some(control);
                    self.process_event(
                        event_loop,
                        Event::EventLoopAwakened(winit::event::Event::NewEvents(
                    ...
                    task::Poll::Pending => {
                        ...
                        Some(control) => match control {
                            ...
                            Control::CreateWindow {
                                ...
                                #[cfg(target_os = "android")]
                                if !self.android_resumed {
                                    log::info!(
                                        "Deferring CreateWindow {id:?} until Android resumed"
                                    );
                                    self.pending_create =
                                        Some(Control::CreateWindow {
                                            id,
                                            settings,
                                            title,
                                            scale_factor,
